...
async function deviceAction(cid,action) {
  ...
  if(action==='files') {
    ...
    $('downloadRemote').onclick=()=>attempt(async()=>{const paths=[...$('remoteFiles').querySelectorAll('[data-remote]:checked')].map(el=>list[Number(el.dataset.remote)].path);if(!paths.length)throw Error('Select recordings first');await api(`cameras/${cid}/download`,'POST',{paths});notice('Selected downloads queued');$('deviceDialog').close();await refresh();});return;
  }
